...
class Dom:
    ...
    def __init__(self, db, dom):
        ...
        for fields in db.fetchall():
            ...
            fdate = datetime.date(int(fields[1][:4]), int(
                fields[1][4:6]), int(fields[1][6:8]))
            ldate = datetime.date(int(fields[2][:4]), int(
                fields[2][4:6]), int(fields[2][6:8]))
...
def main():
    ...
    for c, ip, tdoms in clusts:
        ...
        for dom in c:
            ...
            # , dom.rips, dom.rnets
            print "\t", dom.name, len(dom.rips), len(dom.rnets), dates[0].isoformat(), dates[1].isoformat()

    print "\nCombined Cluster ({0} out of {1}):".format(len(ac), len(rdomList))
    for dom in ac:
        # , dom.rips, dom.rnets
        print "\t", dom.name, len(dom.rips), len(dom.rnets), dom.fdate.isoformat(), dom.ldate.isoformat()
